...
typedef struct cooccur_rec_origin {
    int word1;
    int word2;
    real val;
} CREC_ORIGIN;
...
int origin = 0;//1 for first time run,2 for the second time run.
real eta = 0.05; // Initial learning rate
real alpha = 0.75, x_max = 100.0 ,x_max0 = 100.0 ; // Weighting function parameters, not extremely sensitive to corpus, though may need adjustment for very small or very large corpora
real h = 0.01;//This is the parameter of f1(Xij+delta)=f0(Xij)+h
real *W, *gradsq, *cost, *gradsq0 ;
long long num_lines, *lines_per_thread, vocab_size;
char *vocab_file, *input_file, *save_W_file, *save_gradsq_file,*initvectors,*initgrad;
...
void init_W_Grad_from_file(){
    FILE *fin_w, *fin_grad;
    printf("start init W and grad\n");
    fin_w = fopen(initvectors,"r");
    fin_grad = fopen(initgrad,"r");
    if( fin_w == NULL || fin_grad == NULL)
    {
        fprintf(stderr, "cannot open vectors AND gradsq files\n");
    }
    printf("open init W and grad success\n");

    long long i=0;
    while(1)
    {
        if(feof(fin_w)) break;
        fread(&W[i],sizeof(real),1,fin_w);
        //if(isnan(W[i])||isinf(W[i])) printf("I find the bug\n");
        i++;
    }
    i=0;
    while(1)
    {
        if(feof(fin_grad)) break;
        fread(&gradsq[i],sizeof(real),1,fin_grad);
        i++;
    }
    fclose(fin_w);
    fclose(fin_grad);
    printf("init W and grad success\n");
}
void initialize_parameters() {
    ...
    //  a = posix_memalign((void **)&gradsq0, 128, 2 * vocab_size * (vector_size + 1) * sizeof(real)); // Might perform better than malloc
    // if (gradsq0 == NULL) {
    //     fprintf(stderr, "Error allocating memory for gradsq0\n");
    //     exit(1);
    // }
   ...
   // printf("Num of W %lld\n",a * vector_size + b);
    ...
    vector_size--;
    if(origin == 2) init_W_Grad_from_file();
}
...
void *glove_thread(void *vid) {
    ...
    CREC cr;
    CREC_ORIGIN cr_origin;
    real diff, fdiff, temp1, temp2;
    real M,Lxij,fLxij,M_h,fLxij2,fXij0,M_delta;
    FILE *fin;
    fin = fopen(input_file, "rb");
    if(fin == NULL) {printf("cannot open input file\n");}
    if(origin==0)
        fseeko(fin, (num_lines /num_threads * id) *(sizeof(CREC_ORIGIN)),SEEK_SET);
    else
    ...
    if(W_updates1==NULL||W_updates2==NULL) {printf("cannot Allocate W_updates\n");}
    for (a = 0; a < lines_per_thread[id]; a++) {
        if(origin == 0)
            fread(&cr_origin,sizeof(CREC_ORIGIN),1,fin);
        else
            fread(&cr, sizeof(CREC), 1, fin);
        if (feof(fin)) break;
        //if ( origin==0&&(cr.word1 < 1 || cr.word2 < 1)) { continue; }
        //else if(cr_origin.word1<1||cr_origin.word2<1) continue;
        ...
        l2 = ((cr.word2 - 1LL) + vocab_size) * (vector_size + 1); // shift by vocab_size to get separate vectors for context words
            if (fabs(cr.val)>1e-5){
               //here is the trick
                M = 0;
                M_delta = 0;
                for (b = 0; b < vector_size; b++) M += W[b + l1] * W[b + l2]; // dot product of word and context word vector
                M += W[vector_size + l1] + W[vector_size + l2] ; //calculate M;
                M_delta = M - log(cr.val+cr.Xij_delta);
                M = M - log(cr.val);
                //calc the h
                Lxij = log(1 + cr.Xij_delta / cr.val);
                fXij0 = (cr.val > x_max) ? 1.0 : pow(cr.val / x_max, alpha) ;
                fLxij = fXij0 * Lxij;

                fLxij2 = ((cr.val+cr.Xij_delta)>x_max0) ? 1.0:pow((cr.val+cr.Xij_delta) / x_max0, alpha);

                h =   fLxij2 - fXij0;
                //printf("%lf\n",h);
                //h = 0.0001;
                // h = fXij0 - fLxij2 + 0.01;
                M_h = M * h;//mult delta h(this is crital)
                if(isnan(M) || isnan(Lxij) ||isinf(M)|| isinf(Lxij)) printf("%lf,%lf,%lf\n",M,Lxij,h);
                //fLxij = (cr.val > x_max) ? Lxij : pow(cr.val / x_max, alpha)*Lxij ; // multiply weighting function (f) with diff
               
                //solution 1: h =0.01
                //fdiff = fLxij2*M+M_h-fLxij;//this is the increasing diff.
                //fdiff = -fLx-rij + M_h - h*Lxij; 
                //-fLXij+MH-LH
                // delta 
                fdiff = fXij0 * M -fLxij + M_h - Lxij * h;	
                //zero
                //fdiff = -fLxij;
				//fdiff = fXij0 * M  - fLxij + M_h - Lxij * h;
                //fdiff = A+M_h -fLxij;//this is the increasing diff.
                if (isnan(fdiff) || isnan(fLxij) ||isnan(fLxij2)|| isinf(fdiff) || isinf(fLxij)||isinf(fLxij2)) {
                    fprintf(stderr,"Caught NaN in diff for kdiff ,fLxijfor thread. Skipping update Xij>0\n");
                    continue;
                }
                cost[id] += 0.5 * fLxij2 * M_delta * M_delta;
                
                 //origin
                /*diff = 0;
                for (b = 0; b < vector_size; b++) diff += W[b + l1] * W[b + l2]; // dot product of word and context word vector
                diff += W[vector_size + l1] + W[vector_size + l2] - log(cr.Xij_delta + cr.val); // add separate bias for each word
                fdiff = ((cr.Xij_delta + cr.val) > x_max0) ? diff : pow((cr.Xij_delta + cr.val) / x_max0, alpha) * diff; // multiply weighting function (f) with diff
                
                // Check for NaN and inf() in the diffs.
                if (isnan(diff) ||isnan(fdiff)|| isinf(diff)||isinf(diff)) {
                    fprintf(stderr,"Caught NaN in diff for kdiff for thread2. Skipping update Xij<0\n");
                    continue;
                }
            ...
            else if(fabs(cr.val)<1e-5&&cr.Xij_delta>1e-5){
                /* Calculate cost, save diff for gradients */
                diff = 0;
                for (b = 0; b < vector_size; b++) diff += W[b + l1] * W[b + l2]; // dot product of word and context word vector
                diff += W[vector_size + l1] + W[vector_size + l2] - log(cr.Xij_delta); // add separate bias for each word
                fdiff = (cr.Xij_delta > x_max) ? diff : pow(cr.Xij_delta / x_max, alpha) * diff; // multiply weighting function (f) with diff
          ...
          /* Calculate cost, save diff for gradients */
            l1 = (cr_origin.word1 - 1LL) * (vector_size + 1); // cr word indices start at 1
            l2 = ((cr_origin.word2 - 1LL) + vocab_size) * (vector_size + 1); // shift by vocab_size to get separate vectors for context words
            diff = 0;
            for (b = 0; b < vector_size; b++) diff += W[b + l1] * W[b + l2]; // dot product of word and context word vector
            diff += W[vector_size + l1] + W[vector_size + l2] - log(cr_origin.val); // add separate bias for each word
            fdiff = (cr_origin.val > x_max) ? diff : pow(cr_origin.val / x_max, alpha) * diff; // multiply weighting function (f) with diff
...
int train_glove() {
    ...
    //获取文件大小
    if(origin == 0)
        num_lines = file_size/(sizeof(CREC_ORIGIN));
    else
...
int main(int argc, char **argv) {
    ...
    initvectors = malloc(sizeof(char) * MAX_STRING_LENGTH);
    initgrad = malloc(sizeof(char) * MAX_STRING_LENGTH);
    int result = 0;
    ...
    if (argc == 1) {
        ...
        if ((i = find_arg((char *)"-origin", argc, argv)) > 0) origin = atoi(argv[i + 1]);
        ...
        if ((i = find_arg((char *)"-x-max0", argc, argv)) > 0) x_max0 = atof(argv[i + 1]);
        ...
        else strcpy(input_file, (char *)"shufresult.bin");
        
        if ((i = find_arg((char *)"-initvectors", argc, argv)) > 0) strcpy(initvectors, argv[i + 1]);
        else strcpy(initvectors, (char *)"vectors1.bin");

        if ((i = find_arg((char *)"-initgrad", argc, argv)) > 0) strcpy(initgrad, argv[i + 1]);
        else strcpy(initgrad, (char *)"gradsq1.bin");
